...
// Configurado para apontar para a API Java local.
// No emulador Android, 'localhost' aponta para o próprio emulador. Use '10.0.2.2' para referenciar a máquina host.
  ...
  persist(
    (set, get) => ({
      ...
      login: async (email, senha) => {
        set({ isLoading: true });
        try {
          // Chamada real para API Java (Spring Boot) - descomente quando a API estiver ativa:
          // const response = await api.post('/usuarios/login', { email, senha });
          // const user = response.data;
          // set({ currentUser: user, isLoggedIn: true });
          // get().addLog('sistema', `Produtor ${user.nome} fez login via API`);
          // return true;
          ...
          console.warn('Erro ao conectar com API Java (usando fallback local):', error);
      ...
      register: async (nome, email, senha) => {
        ...
        try {
          ...
          // set(s => ({ registeredUsers: [...s.registeredUsers, newUser], currentUser: newUser, isLoggedIn: true }));
          // return true;
          ...
          console.warn('Erro ao registrar na API Java (usando fallback local):', error);
          const exists = get().registeredUsers.find(u => u.email === email);
          if (exists) return false;
          const newUser: User = { id: uuid(), nome, email, senha, criadoEm: now() };
          set(s => ({
      ...
      updateProfile: async (nome) => {
        ...
        try {
          ...
          // Chamada real para API Java:
          // await api.put(`/usuarios/${user.id}`, { nome });
          ...
          console.warn('Erro ao atualizar perfil na API Java:', error);
        } finally {
          set({ isLoading: false });
        }
      },
      ...
      addLote: async (lote) => {
        ...
        try {
          ...
          console.warn('Erro ao adicionar lote na API Java:', error);
        } finally {
          set({ isLoading: false });
        }
      },
      ...
      updateLote: async (id, updates) => {
        ...
        try {
          // Chamada real para API Java:
          // const response = await api.put(`/lotes/${id}`, updates);
          // const updatedLote = response.data;
          // set(s => ({ lotes: s.lotes.map(l => l.id === id ? updatedLote : l) }));
          ...
          console.warn('Erro ao atualizar lote na API Java:', error);
        } finally {
          set({ isLoading: false });
        }
      },
      ...
      deleteLote: async (id) => {
        ...
        try {
          ...
          console.warn('Erro ao deletar lote na API Java:', error);
        } finally {
          set({ isLoading: false });
        }
      },
      ...
      registrarIrrigacao: async (loteId, ml, tipo) => {
        ...
        try {
          // Chamada real para API Java:
          // const response = await api.post(`/irrigacoes`, { loteId, quantidadeAguaMl: ml, tipoAcionamento: tipo });
          ...
          console.warn('Erro ao registrar irrigação na API Java:', error);
        } finally {
          set({ isLoading: false });
        }
      },
      ...
      deleteIrrigacao: async (id) => {
        ...
        try {
          // Chamada real para API Java:
          // await api.delete(`/irrigacoes/${id}`);

          // Fallback Local
          set(s => ({ historicoIrrigacao: s.historicoIrrigacao.filter(h => h.id !== id) }));
        } catch (error) {
          console.warn('Erro ao deletar registro de irrigação na API Java:', error);
        } finally {
          set({ isLoading: false });
        }
      },
      ...
      addInsumo: async (insumo) => {
        ...
        try {
          ...
          // Fallback Local
          // Garante a presença tanto dos campos categoria/minimo (tipagem original) quanto tipo/quantidadeMinima (telas)
          ...
          console.warn('Erro ao adicionar insumo na API Java:', error);
        } finally {
          set({ isLoading: false });
        }
      },
      ...
      updateInsumo: async (id, updates) => {
        ...
        try {
          // Chamada real para API Java:
          // const response = await api.put(`/insumos/${id}`, updates);
          // const updated = response.data;
          // set(s => ({ insumos: s.insumos.map(i => i.id === id ? updated : i) }));
          ...
          set(s => ({
            insumos: s.insumos.map(i => {
              if (i.id === id) {
                ...
                // Sincroniza campos alternativos
          ...
          console.warn('Erro ao atualizar insumo na API Java:', error);
        } finally {
          set({ isLoading: false });
        }
      },
      ...
      deleteInsumo: async (id) => {
        ...
        try {
          ...
          console.warn('Erro ao deletar insumo na API Java:', error);
        } finally {
          set({ isLoading: false });
        }
      },
      ...
      addColheita: async (colheita) => {
        ...
        try {
          // Chamada real para API Java:
          // const response = await api.post('/colheitas', colheita);
          // const newColheita = response.data;
          ...
          console.warn('Erro ao registrar colheita na API Java:', error);
        } finally {
          set({ isLoading: false });
        }
      },
      ...
      addTarefa: async (tarefa) => {
        ...
        try {
          ...
          console.warn('Erro ao adicionar tarefa na API Java:', error);
        } finally {
          set({ isLoading: false });
        }
      },
      ...
      toggleTarefa: async (id) => {
        ...
        try {
          ...
          if (!tarefa) return;
          // Chamada real para API Java:
          // const response = await api.put(`/tarefas/${id}/toggle`);
          // const updated = response.data;
          ...
          console.warn('Erro ao alterar status da tarefa na API Java:', error);
        } finally {
          set({ isLoading: false });
        }
      },
      ...
      deleteTarefa: async (id) => {
        ...
        try {
          ...
          console.warn('Erro ao deletar tarefa na API Java:', error);
        } finally {
          set({ isLoading: false });
        }
      },
    ...
    }),
    {
      ...
      // Não persiste estado de loading
      partialize: (state) => {
